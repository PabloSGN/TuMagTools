...
def plot_configuration(Folder, selected_sets, polangle):
    ...
    image, _ = read_Tumag(imgs[0])

    fig, axs = plt.subplots(figsize = (12, 12))
    axs.imshow(image, cmap = 'gray', vmin = 200, vmax = 1000)
    for i in range(5):
        axs.plot([Vlines[i], Vlines[i] + 8], [0, 504], c = 'w', ls = '-', lw = 2)
        axs.plot([0, 504], [Hlines[i], Hlines[i] - 8], c = 'w', ls = '-', lw = 2)
        
        axs.plot([Vlines[i] + Boxsize * 1, Vlines[i] + Boxsize * 1 + 8 ], [0, 504], c = 'w', ls = '--', lw = 1)
        axs.plot([Vlines[i] + Boxsize * 2, Vlines[i] + Boxsize * 2 + 8 ], [0, 504], c = 'w', ls = '--', lw = 1)

        axs.plot([0, 504], [Hlines[i] + Boxsize, Hlines[i] + Boxsize - 8], [0, 504], c = 'w', ls = '--', lw = 1)
        axs.plot([0, 504], [Hlines[i] + Boxsize * 2, Hlines[i] + Boxsize * 2 - 8], [0, 504], c = 'w', ls = '--', lw = 1)
